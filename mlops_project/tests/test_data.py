...
class Test_data:
    ...
    def test_labels_shape(self):
        assert all(np.unique(train_loader.dataset.img_labels) == np.arange(0,10)) , "not all 10 classes present"
        assert all(np.unique(val_loader.dataset.img_labels) == np.arange(0,10)) , "not all 10 classes present"
